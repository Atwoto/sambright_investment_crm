...
export function ProductsManager() {
  ...
  const filteredPaints = paints.filter(paint => {
    ...
    const matchesCategory = selectedCategory === 'all' || paint.category === selectedCategory;
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
        <div className="space-y-1">
          ...
          <p className="text-gray-600">Manage your paints inventory and artwork collection</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Add New {activeTab === 'paints' ? 'Paint' : 'Painting'}</DialogTitle>
              <DialogDescription>
                Fill in the details for your new {activeTab === 'paints' ? 'paint product' : 'artwork'}.
              </DialogDescription>
            </DialogHeader>
            ...
            {activeTab === 'paints' ? (
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input
                    id="size"
                    value={newPaint.size || ''}
                    onChange={(e) => setNewPaint({...newPaint, size: e.target.value})}
                    className="col-span-3"
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewPaint({...newPaint, color: e.target.value})}
                    className="col-span-3"
                  />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewPaint({...newPaint, unitPrice: parseFloat(e.target.value)})}
                    className="col-span-3"
                  />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewPaint({...newPaint, stockLevel: parseInt(e.target.value)})}
                    className="col-span-3"
                  />
                </div>
              </div>
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="paintingSize" className="text-right">Size</Label>
                  <Input
                    id="paintingSize"
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="paintingPrice" className="text-right">Price</Label>
                  <Input
                    id="paintingPrice"
                    type="number"
                    value={newPainting.price || ''}
                    onChange={(e) => setNewPainting({...newPainting, price: parseFloat(e.target.value)})}
                    className="col-span-3"
                  />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
            ...
            <DialogFooter>
              <Button onClick={activeTab === 'paints' ? handleAddPaint : handleAddPainting}>
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Edit {activeTab === 'paints' ? 'Paint' : 'Painting'}</DialogTitle>
              <DialogDescription>
                Update the details for this {activeTab === 'paints' ? 'paint product' : 'artwork'}.
              </DialogDescription>
            </DialogHeader>
            ...
            {activeTab === 'paints' ? (
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="edit-medium" className="text-right">Medium</Label>
                  <Input
                    id="edit-medium"
                    value={newPainting.medium || ''}
                    onChange={(e) => setNewPainting({...newPainting, medium: e.target.value})}
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="edit-painting-size" className="text-right">Size</Label>
                  <Input
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="edit-painting-price" className="text-right">Price</Label>
                  <Input
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
        <TabsList className="bg-white/80 backdrop-blur-sm border shadow-sm">
          <TabsTrigger value="paints" className="flex items-center space-x-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-500 data-[state=active]:to-purple-500 data-[state=active]:text-white">
            <Brush className="h-4 w-4" />
            <span>Paints ({paints.length})</span>
          </TabsTrigger>
          <TabsTrigger value="paintings" className="flex items-center space-x-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-500 data-[state=active]:to-purple-500 data-[state=active]:text-white">
        ...
        <TabsContent value="paints" className="space-y-6">
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filteredPaints.map((paint) => (
              <Card key={paint.id} className={`${paint.stockLevel <= paint.minStockLevel ? 'border-orange-200 bg-gradient-to-br from-orange-50 to-red-50' : 'bg-white/80 backdrop-blur-sm'} hover:shadow-lg transition-all duration-200 hover:-translate-y-1`}>
                <CardHeader className="pb-4">
                  <div className="flex justify-between items-start gap-2">
                    <div className="min-w-0 flex-1">
                      <CardTitle className="text-lg truncate">{paint.name}</CardTitle>
                      <CardDescription className="text-sm">{paint.brand} - {paint.size}</CardDescription>
                    </div>
                    {paint.stockLevel <= paint.minStockLevel && (
                      <Badge variant="destructive" className="flex items-center space-x-1 animate-pulse shrink-0">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className={paint.stockLevel <= paint.minStockLevel ? 'text-orange-600 font-semibold' : 'text-green-600'}>
        ...
        <TabsContent value="paintings" className="space-y-6">
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filteredPaintings.map((painting) => (
              <Card key={painting.id} className="bg-white/80 backdrop-blur-sm hover:shadow-lg transition-all duration-200 hover:-translate-y-1">
                <CardHeader className="pb-4">
                  <div className="flex justify-between items-start gap-2">
                    <div className="min-w-0 flex-1">
                      <CardTitle className="text-lg truncate">{painting.title}</CardTitle>
                      <CardDescription className="text-sm">{painting.medium}</CardDescription>
                    </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  {painting.galleryLocation && (
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm">{painting.galleryLocation}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-600">Created:</span>
                    <span className="text-sm">{new Date(painting.dateCreated).toLocaleDateString()}</span>
                  </div>
